...
class ConnectingCodeBloc
    extends Bloc<ConnectingCodeEvent, ConnectingCodeState> {
  ...
  ConnectingCodeBloc(
    ...
    on<LogInWithConnectingCode>(
      _onLogInWithConnectingCode,
      transformer: bloc_concurrency.droppable(),
    );
    on<ReadQrCode>(_onReadQrCode, transformer: bloc_concurrency.sequential());
  }

  FutureOr<void> _onLogInWithConnectingCode(
    LogInWithConnectingCode event,
    Emitter<ConnectingCodeState> emit,
  ) async {
  ...
  Future<void> _onLogInFailure(Emitter<ConnectingCodeState> emit) async {
    ...
    await Future.delayed(const Duration(seconds: 2), () {
      emit(const TryAgainLater());
    });
    await Future.delayed(
      const Duration(seconds: 30),
      () => emit(const ConnectingCodeInitial()),
    );
  }

  FutureOr<void> _onReadQrCode(
    ReadQrCode event,
    Emitter<ConnectingCodeState> emit,
  ) async {
