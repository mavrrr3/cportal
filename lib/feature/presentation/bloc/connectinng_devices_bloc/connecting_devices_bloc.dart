...
class ConnectingDevicesBloc
    extends Bloc<ConnectingDevicesEvent, ConnectingDevicesState> {
  ...
  void _setupEvents() {
    ...
    on<SendScannedData>(_onSendScannedData,
        transformer: bloc_concurrency.droppable());
    on<EndOtherSessions>(_onEndOtherSessions,
        transformer: bloc_concurrency.droppable());
  }

  FutureOr<void> _onLoadConnectingDevices(
      ConnectingDevicesEvent _, Emitter<ConnectingDevicesState> emit) async {
  ...
  FutureOr<void> _onSendScannedData(
      SendScannedData event, Emitter<ConnectingDevicesState> __) async {
    await _sendScannedDataUseCase(
        ConnectingParams(connectingCode: event.scannedData));
    add(LoadConnectingDevices());
  }

  FutureOr<void> _onEndOtherSessions(
      ConnectingDevicesEvent _, Emitter<ConnectingDevicesState> __) async {
